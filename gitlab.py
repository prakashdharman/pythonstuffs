...
def extract_crq_reference(commit_message):
    # Regular expression to match CRQ references like CRQ123456:
    match = re.search(r'CRQ\d{6,}:', commit_message)
    return match.group(0).rstrip(':') if match else 'incompliant'

def clean_date(date_str):
    # Remove the timezone part but keep the standard timestamp format
    return date_str.rsplit(' ', 1)[0]
...
def generate_git_log_file(repo_path, output_log_path):
    ...
    try:
        ...
        # Write the log lines to a log file
        with open(output_log_path, mode='w') as file:
            for line in log_lines:
                date_author_msg = line.split(',', 2)
                if len(date_author_msg) > 2:
                    timestamp = clean_date(date_author_msg[0])  # Use the standard timestamp format
                    author = date_author_msg[1]
                    commit_msg = date_author_msg[2]
                    crq_reference = extract_crq_reference(commit_msg)
                    
                    # Write in log format: <timestamp> Author=..., Commit=..., CRQ=...
                    file.write(f"{timestamp} Author={author}, Commit={commit_msg}, CRQ={crq_reference}\n")
        
        logging.info(f"Log file created: {output_log_path}")
...
def generate_reports(repos, output_dir):
    for repo_path, output_file_name in repos:
        output_log_path = os.path.join(output_dir, output_file_name)
        generate_git_log_file(repo_path, output_log_path)

# Example usage
output_directory = "/path/to/your/output/directory"
repos = [
    ("/path/to/your/first/repo", "commits_first_repo_last_7_days.log"),
    ("/path/to/your/second/repo", "commits_second_repo_last_7_days.log")
]
